...
const Homepage = () => {
  ...
  const checkURL = async (url) => {
    ...
    const response=await axios.post("https://phishing-detection-xg1o.onrender.com/predict/dataURL",{'URL':url})
  ...
  useEffect(() => {
    console.log("first");
        const checkEmail= async(body) => {
    setEmailStatus('processing')
    const response=await axios.post("https://phishing-detection-xg1o.onrender.com/predict/dataEmail" , {'BODY':body})
    console.log(response);
    setEmailVerdict(response.data)
    setEmailStatus("processed")
    return "email done"
